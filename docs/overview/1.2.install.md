...
Since Uno is using the `Shared` project in all the platforms, all it takes to run this project on Wasm is to install LiveCharts2 on
...
Simple, just repeat the previous step in the rest of the projects in the platforms folder!, and we are done we have
a cross platform chart!.
