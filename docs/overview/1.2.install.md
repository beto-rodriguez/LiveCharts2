...
At this point there is only experimental support for Uno platform, you can use LiveCharts in a uno platform application
by cloning the repo.

> git clone https://github.com/beto-rodriguez/LiveCharts2.git

And then fetch and checkout the `uno-platform` branch.

> git fetch
> git checkout uno-platform

In your current directory browse for `LiveCharts2/src/skiasharp/LiveChartsCore.SkiaSharpView.Uno`, finally add a reference to the
`LiveChartsCore.SkiaSharpView.Uno.csproj` in all your Uno application taget projects.

Now you are ready to add a chart as described in the next steps:

## Create a View Model

After the package is referenced, add a new class to your project as follows:
...
Finally go to the `Shared` project and open the `MainPage.xaml` file, add the LiveCharts namespace, don't forget to add also the
namespace of your `ViewModel` class, then add a `CartesianChart` control and bind the `Series` property:
...
Finally in the root folder browse for `MauiProgram` class, and register SkiaSharp in the application:"
