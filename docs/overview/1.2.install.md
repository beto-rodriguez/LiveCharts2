...
When the theme is not explicitly defined, LiveCharts adds the [default theme](https://github.com/beto-rodriguez/LiveCharts2/blob/master/src/skiasharp/LiveChartsCore.SkiaSharp/ThemesExtensions.cs), 
but we can also use the default theme as a base and change the base style of the charts in our app, 
in the next example, we add the default theme to the app using the `AddDefaultTheme()` extension method, 
as the parameter we pass the additional configuration for the theme, in this case we use the `OnInitialized()` 
method to define the animations speed, easing function and color. To learn more about themes, please visit the 
[themes article](https://livecharts.dev/docs/{{ platform }}/{{ version }}/Overview.Themes).
...
{ // mark
    config.AddDefaultTheme(theme => theme
        .OnInitialized(() =>
        {
...
You can plot anything in a chart as soon as you let the library how to handle that object; In the next example,
we register a mapper for the `City` class, when a chart uses the `City` class as the data source, it will use this
mapper to build the chart coordinates.
